...
export function ActiveSession({ initialConfig, onEndSession, onDiscard }) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      // Calculate elapsed based on start time to avoid drift, but we need to account for pauses if we implemented them fully.
      // For now, since we only "pause" during review, we can just update elapsed if not reviewing.
      // However, if we use Date.now() - startTime, it will jump when we return from review.
      // So we should update startTime when returning from review, OR just increment elapsed.
      // Incrementing is safer for simple pausing.
      setElapsed(e => e + 1);
    }, 1000);
  ...
  const handleSave = (finalSessionData) => {
    onEndSession({
      ...initialConfig,
      ...finalSessionData,
      startTime,
  ...
  const handleCancelReview = () => {
    ...
    // No need to adjust startTime if we are just incrementing elapsed.
    // The timer will just resume ticking from where it left off.
  };
